...
截至 2026-09-07，MOD-01/02/03 已评审并集成，MOD-25 的地理编码与缺省日期修复已通过 PR #12 合并；MOD-04/05 仍处于评审边界，桌面 PR #18 已合并但 MOD-11/12/13 仍有真实事件、托盘和配置闭环待完成。ARCH-01 已通过 PR #16 合并，当前下一项架构工作是将 Agent/Model 编排移入 Runtime Application 层。PR #8 已通过代码评审但等待负责人解决合并冲突，PR #19 等待非作者评审。SDK 为 0.1.0-alpha.1，协议仍未冻结；尚无完整生产闭环。完整进度见 ROADMAP。

## 文档入口

- [产品需求 PRD](docs/PRD.md)：产品范围、需求编号、优先级和验收条件。
- [架构设计](docs/ARCHITECTURE.md)：模块、进程、协议、数据与技术验证项。
- [模块分工](docs/MODULE_ASSIGNMENTS.md)：26 个独立模块、GitHub 负责人和文件所有权；`goo122` 为 A，`zemeng` 为 B，`Potatos498` 为 C。
