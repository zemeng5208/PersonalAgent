...
普通启动时，提交文字任务会通过本地 `TaskRuntime` 调用 `@personal-agent/runtime/text` 的 Runtime Application，由该层执行 Agent、`ModelGateway` 和 Provider 编排。Desktop 主进程只负责安全配置、IPC 和任务展示；没有 Provider 时任务会如实失败，不会静默生成假回答。
