...
- **简体书写的外国地名是「被检出并标注」，不是「被纠正」**：默认的 `ranked` 模式仍会返回 伦敦/安大略、东京/江苏、罗马/昆士兰，只是带上 `confidence: 'low'`。根因是 Open-Meteo 的 `language` 参数同时决定「搜哪套名字」——任何中文串在 `language=en` 下恒返回 0 条（繁体 `東京`/`倫敦` 也一样，`zh-TW`/`zh-Hant` 同样为空），而 `zh` 名字集的简繁覆盖逐条不可预测，没有系统性改写规则。真正纠正要靠调用方给 `locationQuery`（本轮实测 6 个全部救回）。彻底修复需要简繁映射表或第二地理编码源：前者与「无新增外部依赖」冲突；后者本轮实测不可行——`nominatim.openstreetmap.org`、`api.bigdatacloud.net` 从本机连接超时（无法验证），唯一连得上的 `photon.komoot.io` 中文覆盖更差（`纽约`→山东临沂一个住宅小区、`首尔`→河南一家餐厅、`伦敦`→云南一条街），且 `lang=zh` 直接 HTTP 400。两条路都不做，也不用猜测掩盖。
...
- 每个未命中缓存的地名产生 1–3 次地理编码请求：配置语言以 `en` 开头且输入无汉字时 1 次；否则「配置语言 + `en`」2 次；输入含汉字且配置语言不是 `zh` 时 3 次。带 `locationQuery` 时提示串按同样的规则再跑一轮集合，但**只在原始输入零候选或判 `low` 时才发**。命中缓存后为 0 次，预报请求次数不变。
