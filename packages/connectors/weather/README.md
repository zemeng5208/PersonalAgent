...
默认接 **Open-Meteo** 真实提供商（免密钥、无账号），manifest `verification` 为 `conditional`——结果依赖出站网络可达。`FakeWeatherProvider` 夹具仅用于离线测试，`verification` 为 `mock`。

## 导出入口

`@personal-agent/weather`（ESM，类型声明在 `dist/index.d.ts`）：

- `register(host, options?)`：向 ToolHost 注册 `weather.forecast` 工具（scope `weather:read`），返回 dispose。未指定 `provider` 时使用 `OpenMeteoProvider`。`options`：`provider`、`now`、`defaultLocation`、`cacheTtlMs`（默认 10 分钟）、`language`（默认 `zh`）、`locationResolution`（默认 `ranked`）。
- `OpenMeteoProvider`：真实提供商。`options`：`fetchImpl`（注入以便离线测试）、`language`、`locationResolution`、`forecastBaseUrl`、`geocodingBaseUrl`、`geocodeCacheLimit`（默认 500）。
...
本轮结果：5 个工作区类型检查全部通过；全仓 53 项测试，52 通过、1 跳过（weather 27：26 通过 + 1 项真实读回默认跳过；testkit 13、client 5、contracts 4、storage 4）。

离线测试（默认运行，无网络）：27 项中的 26 项。其中 Open-Meteo 相关 15 项用注入的 `fetchImpl` 覆盖时间来源标注、夏令时日界、歧义披露、`strict` 拒绝、地理编码缓存、单位参数、降水缺失、未知天气代码、错误码映射、网络失败与取消、畸形响应、空地点拒绝、manifest `conditional`，以及经 `FakeToolHost` 的 ajv 实际校验工具输出 schema（含 `resolved` 与可空降水概率）。
...
未设 `PA_WEATHER_LIVE=1` 时该测试跳过，保证 CI 与离线环境确定性。本轮实际执行结果见下方「真实读回证据」。

根 `npm run check` 当前无法整体通过：它在第一步 `packages/contracts` 的 `check:generated` 即失败（提示生成文件陈旧）。该失败已用 `git stash` 在不含本包任何改动的干净 main 检出上复现，也在 main 的 CI 运行 34006560474、34006694534 中出现，属既有问题、与本包无关，需 `goo122` 重新生成或修正漂移检查。本包未改动 `packages/contracts/` 下任何文件。

## 真实读回证据

2026-09-06 于 Windows 本机对生产端点实际执行（`PA_WEATHER_LIVE=1`），查询「北京」当日：

```json
{
  "occurredAt": "2026-09-05T16:00:00.000Z",
  "fetchedAt": "2026-09-06T03:36:29.709Z",
...
- 未实现 MOD-05 权限隔离，scope 校验目前由 testkit 的 `FakeToolHost` 承担；`register(host)` 尚未接入应用根装配（归 `goo122`）。
