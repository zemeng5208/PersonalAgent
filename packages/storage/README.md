...
负责人 goo122；评审者 zemeng。提供可信宿主使用的 SQLite 连接与有序迁移，不实现任务状态机或公共 StoragePort。
...
此连接只供可信根装配使用，不交给第三方模块。后续 MOD-02/MOD-05 定义并注入受限 StoragePort；当前底座不是模块权限隔离机制。业务请求使用参数绑定，模型、UI 和工具不得提交 SQL。
...
- 尚无 PR、交叉评审和合并证据。PA-004 的任务持久化、取消与恢复仍需 MOD-03 验收，不能由本包测试替代。
