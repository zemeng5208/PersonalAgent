...
| 表面 | 当前可核对证据 | 证据边界 |
| --- | --- | --- |
| 云端构建与版本化 | [2026-09-12 控制台读回](2026-09-12-multi-agent-build.json)：多智能体和三个子工作流提交版本；2026-09-24 只读预览最新已发布多智能体 `v20260917160941` | 已发布版本仍显示六个工作流卡和控制器一个“未配置”槽；未从 trace 证明该槽是否进入执行路径 |
| 部署与 API 渠道 | 2026-09-24 控制台读回：API 渠道已发布；主运行时状态正常，`Latest` 访问方式指向运行时 `v6` | 应用发布版本与运行时版本是两套编号；尚缺版本绑定的失败/回滚与账单读回 |
| 已发布部署的调用 | [有界多次云调用工具 MVP](TOOL-INVOCATIONS-MVP.md)：三次独立 published API 合成探针；另有同一本地 source task 的两次云请求均 HTTP 200，并按同会话、输入和时间匹配到两条成功平台 trace | 平台未展示调用方 `X-Request-Id`，不能宣称精确 ID join；列表输出列为 `{"output":null}`，响应正文仍以本地 SSE 收据为准 |
...
### 2026-09-24 平台只读读回与下一验收包

用户已登录的 `cn-southwest-2` 控制台显示：当前编辑态标为“未发布”，保存于
2026-09-17 16:09:27；最新应用发布版本 `v20260917160941`（版本 ID
...
1. **角色编排与发布配置**：核对六个卡与“未配置”槽的实际路由含义，清理经确认
   无用的重复引用；在固定输入下读回起始/默认/结束角色的真实交接 trace、失败分支
   和精确发布版本。现有 trace 只证明三个 `UserInput`/模型 span，不证明每个角色按设计执行。
...
3. **部署验收与回滚**：将应用发布版本、运行时 `v6`、`Latest` 端点和每条正式调用
   建立可核对关联；验证健康/失败、降级不静默切 Local、版本回退及账单费用读回。
   当前只有状态正常、API 已发布和成功 trace，不能推断回滚已可用或费用金额。
