...
版本：0.3 · 日期：2026-09-05 · 状态：MOD-01/02 已本地实现并联合测试，待评审与集成，其他模块未启动
...
| `Potatos498` | 第三位协作者（2026-09-06 登记） | 日程、邮件、订阅、搜索、天气与社交连接器 |
...
双人模式：`goo122` 与 `zemeng` 共同开发。待认领模块按小工作包划分后，分别转给 `goo122` 或 `zemeng`；增加第三位协作者时先登记 GitHub 用户名，再分配对应子模块。不能两人同时持有同一模块。
...
`zemeng` 的改动由 `goo122` 或已登记的第三位协作者评审，`goo122` 的改动由 `zemeng` 或已登记的第三位协作者评审；本人不能充当自己的独立评审。MOD-01 已获用户开工授权，具体状态以 ROADMAP 为准。

## 7. 开工门槛

1. `goo122` 先完成 MOD-01 和 MOD-02：可启动底座、契约版本、fake Runtime、消费者示例及兼容性检查。
2. `zemeng` 验证 fake 客户端；待认领的连接器负责人验证 fake 工具/连接器；最小契约形成冻结记录。
3. `zemeng` 从 MOD-11/12/13 开始；`goo122` 继续 MOD-03/04/05 和 MOD-08；待认领模块负责人从 MOD-25 或 MOD-22 开始。各自可以使用 fake 依赖，不需要等所有功能完成。
