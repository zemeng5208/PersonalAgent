...
版本：0.4 · 日期：2026-09-07 · 状态：MOD-01/02/03/25 已按当前验收边界集成；MOD-04/05 保持 review，桌面 MOD-11/12/13 继续 in_progress

本文件是模块负责人、文件所有权和交付边界的唯一登记处。[PRD](PRD.md) 定义需求，[公共开发协议](DEVELOPMENT_PROTOCOL.md) 定义互通契约，[目录规范](PROJECT_STRUCTURE.md) 定义模块布局，[ROADMAP](ROADMAP.md) 维护执行状态。
...
- 下表路径同时用于规划与已实现模块。生产源码只放 `apps/`、`packages/` 及登记的扩展目录；根级 `src/` 不再作为第二套入口。未开工模块不创建空目录。
...
`zemeng` 的改动由 `goo122` 或 `Potatos498` 评审，`goo122` 的改动由 `zemeng` 或 `Potatos498` 评审；本人不能充当自己的独立评审。MOD-01/02/03 已完成评审集成；MOD-25 的 PR #4、PR #9、PR #12 已合并，离线与真实条件性读回验收证据已登记，具体限制和模块状态以 ROADMAP 为准。桌面 PR #18 已合并，但不把桌面里程碑等同于 MOD-11/12/13 全部完成。
