...
| MOD-23 | M3 | in_progress | `Potatos498` / 分支 `feat/mod-23-notifications`，评审者 `goo122`（非作者）/ 用户 2026-09-07 明确授权启动。独占目录 `packages/notifications/`；生产依赖仅 MOD-02 contracts，调度经结构兼容建议交给 MOD-03，界面交 `zemeng`。范围见下方变更记录的登记条目 |
