...
| MOD-20 | M2 本地提醒、M3 日历 | review | `Potatos498` / 分支 `feat/mod-20-productivity`，指定 `goo122` 非作者评审 / 用户 2026-09-07 明确授权启动；模块设计见 [mod-20](modules/mod-20.md)。实测：根 `npm run check` 退出码 0（含架构门禁），全仓 157 项为 154 通过＋3 项真实读回默认跳过，其中 productivity 14 项、calendar 10 项全部离线通过（Fake 时钟/日历注入）；DST 四边界（纽约/悉尼的跳跃与回拨）经 `localToUtc` 单测固定；`respond` 幂等与 dedupeKey 变更换键有具名测试。真实日历提供商授权读回为独立后续工作包，manifest 保持 `mock`；Runtime 接线归 `goo122`，接线前不得转 done |
