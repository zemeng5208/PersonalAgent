...
| MOD-24 | M2 | review | `Potatos498` / PR #47；OpenAlex 学术源（免 key）＋Fake；三披露（来源/发布时间/获取时间）＋失败/过期区分（缓存三态、stale 标注）；signal 透传；JSON 编码缓存键；离线 14＋live 15 全过。goo122 复审意见全部修复，待复审合并 |
