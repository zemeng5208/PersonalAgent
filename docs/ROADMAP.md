...
| MOD-22 | M3 | review | `Potatos498` / 分支 `feat/mod-22-feeds` / 用户 2026-09-06 明确授权启动；依赖 MOD-02 已集成；MOD-05 已于 main `f69a745` 交付，但本包未接入其 `ToolGateway`，`feeds:read` 只经 testkit `FakeToolHost` 验证。实测：根 `npm run check` 退出码 0，全仓 156 项为 154 通过＋2 项真实读回默认跳过（feeds 74 项）；`PA_FEEDS_LIVE=1` 下阮一峰 Atom 与少数派 RSS 各两轮，状态序列 `[200, 304, 200, 200]`。根装配接线归 `goo122`，未接线前不得转 done |
...
- 2026-09-06：MOD-22 订阅采集实现完成，台账由 `in_progress` 改为 `review`，M3 里程碑状态、开工边界第 6 条与工作包的交付／验收／证据／限制同步更新。已合并 main `fae0706`（带入 PR #6 的 MOD-25 合并后审计与 PR #7 的 MOD-05 三个包），三处冲突（根 `package.json`、`package-lock.json`、本文档）均保留双方记录后解决，未使用 `reset --hard`、强制推送或清理未跟踪文件；根 `build` 脚本与锁文件现覆盖 9 个工作区。实测：根 `npm run check` 退出码 0，全仓 156 项为 154 通过＋2 项真实读回默认跳过，其中 feeds 74 项（73 通过＋1 跳过）；`PA_FEEDS_LIVE=1` 对两个生产端点做了三次完整读回（`08:26:39Z`、`08:34:25Z`、合并后 `08:45:03Z`），每次恰好 4 个请求、状态序列同为 `[200, 304, 200, 200]`。新增依赖 `fast-xml-parser` 5.11.1（MIT、6 个传递依赖、该子树零告警）已核实。**更正两条计划阶段的前提**：该库确实带 `DocTypeReader`，因此实体安全来自本包关闭其 `processEntities` 并改用固定表解码，而非「库无 DTD 能力」；少数派有 channel 级 `<pubDate>`，登记时记为无 feed 级时间不完整。ROADMAP 相对链接与模块编号一致性检查通过。
- 此记录不构成任何运行时能力通过证明。
...
- 负责人：`Potatos498`；评审者：`goo122`；用户 2026-09-06 明确授权启动；实现已完成于分支 `feat/mod-22-feeds`，等待非作者评审。
