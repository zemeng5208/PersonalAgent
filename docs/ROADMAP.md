...
| MOD-23 | M3 | review | `Potatos498` / 分支 `feat/mod-23-notifications`，指定 `goo122` 非作者评审；模块设计见 [mod-23](modules/mod-23.md)。交付 `packages/notifications`：`ConnectorItem` 事件流按用户规则裁定（立即/安静持有/暂停持有/聚合摘要），安静窗口 IANA 时区 DST 安全且支持跨午夜，暂停过期自动恢复，聚合按窗口关闭或条数上限触发且交付尊重安静时段，dedupeKey 去重防重复通知；`planSchedules` 产结构兼容 Runtime `ScheduleInput` 的调度建议（不自建调度器，ADR-0002）；工具 `notifications.status` 只读。实测：根 `npm run check` 退出码 0（含架构门禁），notifications 12 项全过（全离线 fake 时钟）。与 feeds/calendar 事件流及摘要展示的接线归 `goo122` 装配，接线前不转 done |
...
- 2026-09-07：PR #27 修复通知恢复语义与 DST 跳时（第一阶段修复计划）。交付：批次生命周期 pending → ready_for_delivery → delivered——drain 裁定即持久化批次，桌面 acknowledge(batchId) 确认后才置 delivered，确认前 drain 原样返回未确认批次（崩溃重启不丢通知、不重复生成；条目级 delivered 标记防重启后再裁）；已确认批次保留最近 100 条作幂等记录；status() 新增 unacknowledgedBatches 披露。nextQuietEndMs 改为扫描「首个离开安静区的整分」——修复春季跳时当天 endLocal 不存在（纽约 2026-03-08 22:00–02:30 窗口的 02:30 被跳过）时扫 26 小时返回 null 的缺陷，跳跃夜于 03:00 EDT 释放，回拨夜于 02:30 唯一一次出现时释放。验收测试 +3（崩溃恢复取回相同批次/确认后重启不重复+ack 幂等与 NOT_FOUND/DST 春季跳跃与回拨夜），既有测试按新恢复语义更新 2 处断言（未确认批次会被再次返回）。实测：notifications 15 项全过，根 npm run check 退出码 0，全仓 172 项 168 通过＋4 门控跳过。ROADMAP 相对链接检查通过。
- 此记录不构成任何运行时能力通过证明。
