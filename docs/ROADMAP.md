...
| M1.6 持续认知创新 | 版本化世界状态、Goal/Event、目标/事实/决策图谱、最小计划修复 | 事实变化产生可回放影响；AgentArts 只更新受影响计划；Evidence 闭环 | MOD-27/28 离线领域核心及图谱持久化已集成；事实流、消费者纵向接线、AgentArts 与 Evidence 闭环未完成 |
...
| MOD-24 | M2 | review | `Potatos498` / 分支 `feat/mod-24-research`，指定 `goo122` 非作者评审；模块设计见 [mod-24](modules/mod-24.md)。交付 `packages/connectors/research`：OpenAlex 学术源（免 key，实测可达）＋Fake 夹具；材料三披露（来源/发布时间/获取时间）；**失败/过期区分**（错误照实、缓存三态 fresh/fetched/stale+lastError、材料过旧标 stale 仍返回）；无发布时间回退抓取时刻并显式标注；工具 `research.search` 只读（scope `research:read`）。实测：research 9 项（8 离线＋1 门控 live，`PA_RESEARCH_LIVE=1` 全过含真实 OpenAlex 读回）。通用 web 搜索（Bing 需 key）与网页抓取为后续提供商工作包；Runtime 接线归 `goo122` |
...
- 2026-09-08：登记 MOD-24 搜索与资料获取工作包（分支 `feat/mod-24-research`）。用户本轮明确授权启动并选定方案：Fake 提供商＋OpenAlex 学术源（实测免 key 可达，HTTP 200，响应含 publication_date/venue/authors/doi）。按 PROJECT_STRUCTURE §11 登记要素：负责人 `Potatos498`、评审者 `goo122`、独占目录 `packages/connectors/research/`、生产依赖仅 `@personal-agent/contracts`（HTTP 走注入 fetch，无新外部依赖）。公共输入输出：查询 → 材料（`ConnectorItem`：`occurredAt`=发布时间，仅日期精度归一为当日 UTC 零点并在 contentRef 标注；缺失时回退获取时刻并显式标注；`sensitivity: 'public'`；`validFor`＝[发布时刻, 发布时刻+maxAgeMs] 时效窗口）＋每条材料的 freshness 披露（fresh/stale，maxAgeMs 默认 365 天可配置）。失败/过期区分（PA-010 验收核心）：检索失败照实抛错不用旧结果冒充；TTL 缓存过期或提供商失败时回退 stale 并附 lastError（weather 同款三态）；材料过旧仍返回但标 stale 由 MOD-04/用户取舍。工具 `research.search` 只读（scope `research:read`）。数据源侦察实测（2026-09-08）：Wikipedia/DuckDuckGo/Brave/searx 从本机连接层不通，Mojeek 403，Bing API 端点可达但需 Azure key——Bing 与 HN Algolia 作为后续提供商工作包，不在本包。Fake 验收：固定响应夹具（含新鲜/过旧/无发布日期三形态）、freshness 三态、错误映射、契约校验；真实验收：OpenAlex live 读回（`PA_RESEARCH_LIVE=1` 门控）。本条只是登记，MOD-24 尚无实现、无测试证据。
- 2026-09-13：MOD-24 实现完成，台账改 `review`，模块设计补 [mod-24](modules/mod-24.md)。交付 `packages/connectors/research`（`@personal-agent/research`）：`ResearchProvider` 可插拔端口＋Fake 夹具（新鲜/过旧/无发布日期三形态）＋`OpenAlexProvider`（免 key，`/works?search=` 映射 id/display_name/publication_date/authorships/venue/doi，429→`RATE_LIMITED`、网络→`EXTERNAL_FAILURE`、取消不吞）＋`ResearchService`（三披露与失败/过期区分：`occurredAt`=发布时刻且日期归一当日 UTC 零点、缺失回退抓取时刻并标 `fetched_fallback`；缓存三态 fresh/fetched/stale＋lastError，可重试失败回退 stale、不可重试照实抛；材料过旧标 `freshness:'stale'` 仍返回，`validFor`=[发布, 发布+maxAgeMs]，默认 365 天）＋ConnectorPort（search-only，其余 `UNSUPPORTED_CAPABILITY`）与只读工具 `research.search`（scope `research:read`）。根 `package.json` build 一行＋锁文件纯工作区链接归 `goo122` 集成确认。实测：research 9 项＝8 离线（契约/失败过期区分/回退标注/缓存三态含预热-越期-注入序列/规范化含坏条目/能力边界/工具校验）＋1 门控 live，`PA_RESEARCH_LIVE=1` 下 9/9 全过（真实 OpenAlex 读回，externalId 均为 W-id 且过契约校验）。ROADMAP 相对链接检查通过。
