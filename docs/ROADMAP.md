...
| MOD-21 | M3 | review | `Potatos498` / 分支 `feat/mod-21-mail-qq`（第一个提供商：QQ 邮箱），指定 `goo122` 非作者评审；模块设计见 [mod-21](modules/mod-21.md)。交付 `packages/connectors/mail`：QQ 形状 Fake＋`QQMailProvider`（imapflow/nodemailer，新增外部依赖待 goo122 确认）；增量游标 `uidValidity:lastUid`、`CURSOR_EXPIRED`、`mark_seen` 幂等、**send 超时→unknown 先核对不盲重试**（PA-014）；条目 `sensitivity:'private'` 不含正文；工具 `mail.inbox` 只读。实测：根 `npm run check` 退出码 0，全仓 164 项 159 通过＋5 门控跳过（mail 12 项离线全过）。**QQ 真实链路待授权码 live 验证**（门控命令见包 README）；Runtime 接线归 `goo122`，接线与真实验收前不转 done |
