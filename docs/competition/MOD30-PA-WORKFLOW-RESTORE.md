...
`cn-southwest-2` AgentArts 控制台只读抄录现有源工作流编辑器内容和控制器引用。
本文件是恢复材料，不表示本次修改、提交或部署了云资源。编辑器显示“已提交”，
但以下文字与运行实例当前绑定版本的逐字一致性仍须在恢复时读回版本快照及 trace。

| 路由 | 源工作流 ID | 控制器画布引用的版本 ID | 当前编辑器保存时间 |
| --- | --- | --- | --- |
