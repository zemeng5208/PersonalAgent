...
- 基线：`main@72cc76b1f07d74ecbf431f1b31bcb62d1e403449`。
- 分支：`codex/zemeng/memory-query-fake`；负责人：`zemeng`；状态：`review`。
- 非作者评审者：`goo122`；集成追加根构建、lock workspace 登记与接口目录，无外部依赖。
- 独占范围：`packages/memory/**` 与本文档。
...
- `FactChangeFeed`、ack、sequence、持久 checkpoint 和重启续读留到下一工作包；在“什么
  持久完成点允许 ack”确认前不提前冻结接口。
...
本工作包不运行全仓测试、真实数据、云端、GUI 或生产迁移。

实际执行：包构建通过，定向测试 6/6，架构检查 21 workspace 与差异检查通过。
本机为 Node 26.3/npm 11.16；目标 Node 24.15/npm 11.12 由干净 CI 检查。
