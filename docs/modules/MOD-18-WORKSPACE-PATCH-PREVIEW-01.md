...
```powershell
node <node24> <typescript>/tsc -p packages/contracts/tsconfig.json
node <node24> <typescript>/tsc -p packages/coding-tools/tsconfig.json
node <node24> --test --test-isolation=none packages/coding-tools/test/workspace-patch-preview.test.mjs
node <node24> --test --test-isolation=none tests/integration/workspace-patch-preview-policy.test.mjs
git diff --check
```

模块测试覆盖精确 hash、非 ASCII、输入快照、编辑唯一性与顺序、BOM/UTF-8/控制字符、路径/敏感/link、上限和生命周期。Policy 集成覆盖未授权零执行、参数摘要绑定、撤销、候选 hash，以及文件与目录未变化。均为本地合成证据；生产装配、真实工作区和 AgentArts 仍未验证。
