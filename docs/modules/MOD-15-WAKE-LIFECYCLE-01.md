...
- 工作树：`.worktrees/zemeng-voice-wake-lifecycle`；分支：`codex/zemeng/voice-wake-lifecycle`。
- 实现范围：`packages/voice-wake/**` 与本文件；主控另行维护根构建、工作区锁登记及接口目录，不修改 MOD-14、Runtime、contracts 或公共 wire。
- 状态：`review` / provisional；离线实现不等于接口冻结或模块 done。
...
误触率或回声指标。期限和调用方 `AbortSignal` 同时贯穿授权检查与源订阅。
...
本次目标验证结果：workspace `build` 与 `typecheck` 通过；默认 `npm.cmd test` 在当前 Windows
沙箱由 Node test runner 创建隔离子进程时命中 `spawn EPERM`，因此使用同一测试文件的
`node --test --test-isolation=none packages/voice-wake/test/voice-wake.test.mjs` 完成验证，
19/19 通过。该替代只关闭测试文件子进程隔离，不改变产品代码路径。

测试覆盖默认零订阅、显式启用单订阅、重复启用、撤销/过期/断设备/关闭/销毁、旧 epoch
迟到回调、播放/冷却抑制、deadline/cancel、非合作授权、固定错误脱敏，以及生命周期
初始/终态、冻结、去重、退订、异常隔离和同步源失败。测试不是实机误触、回声或 ASR
准确率证据；本次未运行真实音频、云服务、Electron 或 MOD-14 装配。主控已将该包加入根
build，根 lock 仅新增该 workspace 与 link，无外部依赖变动；架构边界检查 1/1 通过。
本机 Node 26.3/npm 11.16 与仓库要求的 Node 24.15/npm 11.12 不同，声明版本验证以 CI 为准。

本模块由 `zemeng` 实现并交由 `goo122` 进行非作者评审；本代理自测不能替代该批准。
